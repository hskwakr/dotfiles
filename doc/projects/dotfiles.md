...
## common と OS 固有設定の優先ルール

`install.sh` は dotfiles を 2 段階でリンクします。

1. `env/common/` のファイルを `$HOME` にリンク
2. `env/<OS>/` のファイルを `$HOME` にリンク（後勝ち）

同名ファイルが `env/common/` と `env/<OS>/` の両方にある場合、**OS 固有版が最終的なリンク先**になります。これは「後勝ち（last-wins）」方式で実現されています。

### 動作の詳細

- common の段階で `~/.tmux.conf → env/common/.tmux.conf` がリンクされる
- OS 固有の段階で、既存のリンク先が異なることを検知し、`env/macOS/.tmux.conf` へ付け替える
- ログに `Overriding common config with OS-specific version` と出力される

### バックアップとの関係

- `$HOME` にリンク前の実ファイルがあった場合、common の段階で `backups/original/` に保存される
- OS 固有での上書き時はシンボリックリンクの付け替えのみで、バックアップには影響しない
- `clean.sh -r` による復元は `backups/original/` から行うため、オーバーライドの有無に関わらず正しく復元される

### 設計上の制約

- OS 固有版は common をマージ（拡張）するのではなく、**完全に置き換える**ため、common の内容も含めた完全版として用意する必要がある
- 実行順序（common → OS 固有）を入れ替えると挙動が逆転するため、`main()` 内の呼び出し順序を変更してはならない
